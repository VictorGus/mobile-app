...
create table if not exists notification (id text primary key not null, user_id text, n_action text, notification_rate bigint, date_time timestamp, CONSTRAINT fk_user FOREIGN KEY(user_id) REFERENCES public_user(id) ON DELETE SET NULL);

create table if not exists notification_result (id text primary key not null, notification_id text, date_time timestamp, n_result text, CONSTRAINT fk_notification FOREIGN KEY(notification_id) REFERENCES notification(id) ON DELETE SET NULL);
